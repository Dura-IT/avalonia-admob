...
{
    /// <summary>
    /// iOS implementation of <see cref="IRewardedAdService" />: loads and presents a native AdMob
    /// <see cref="GADRewardedAd" />, gating on UMP consent through <see cref="AdMobInitializer" /> and
    ...
    internal sealed class RewardedAdService : IRewardedAdService
    {
        ...
        public async Task LoadAsync(string? adUnitId = null)
        {
            string resolvedAdUnitId = AdUnitResolver.ResolveOrThrow(
                adUnitId,
                AdMobRuntime.Options.RewardedAdUnitId?.IOS,
            ...
            GADRewardedAd.Load(resolvedAdUnitId, GADRequest.Request(), OnLoadCompleted);
        }
        ...
        [SuppressMessage(
            ...
            Justification = "The content delegate's lifetime transfers to the native GADRewardedAd; disposing it here would break the presentation callbacks."
        )]
        public Task<AdReward?> ShowAsync()
        {
            ...
            ((IGADFullScreenPresentingAd)_ad).Delegate = new RewardedContentDelegate(this);
            _ad.Present(viewController, OnUserDidEarnReward);
            return _showCompletion.Task;
        }

        private void OnLoadCompleted(GADRewardedAd? ad, NSError? error)
        {
        ...
        // Bridges the native full-screen presentation callbacks into the service. The GADRewardedAd owns
        // this delegate until it is released.
        private sealed class RewardedContentDelegate : GADFullScreenContentDelegate
        {
            private readonly RewardedAdService _service;

            public RewardedContentDelegate(RewardedAdService service) => _service = service;
