...
public partial class NativeAd : NativeControlHost
{
    ...
    )
    {
        var adLoader = new AdLoader.Builder(context, adUnitId)
            .ForNativeAd(new NativeAdLoadedListener(this, adView, views, style, logger, adUnitId))
    ...
    )
    {
        ...
        {
            drawable.SetColor(Color.Argb(background.A, background.R, background.G, background.B));
        }
